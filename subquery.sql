...
-- SELECT ALL THE EMPLOYEES WHOSE DEPARTMENT IS LOCATED AT TORANTO
...
SELECT LOCATION_ID FROM locations WHERE CITY= 'Toranto';
SELECT DEPARTMENT_ID FROM departments WHERE LOCATION_ID IN (1800);

SELECT * FROM employees WHERE DEPARTMENT_ID IN (
SELECT DEPARTMENT_ID FROM departments WHERE CITY = LOCATION_ID IN
 SELECT LOCATION_ID FROM Locations WHERE CITY = 'Toranto'));
 
 -- This query retries WHOSE Salary is below the Average salary Of All Employees In The Employees
 
 SELECT avg(Salary) from employees;
 SELECT * FROM employees WHERE Salary < (Select avg(salary) FROM employees);
 
 -- /* LIST ALL EMPLOEES WHOSE SALARY IS LESS THAN THE SALARY OF ALL THE EMPLOYEES WORKING FOR DEPARTMENT_ID 30 */
 ...
 SELECT Department_ID FROM departments WHERE LOCATION_ID In(1800);
 SELECT * FROM employees WHERE DEPARTMENT_ID IN(20);
 
 This Query Retrieves all employees whise salary is below the average Salary of all employees in the employee table ----
 
 Select avg((salary) from employees;
 select * from employees WHERE SALARY < (select avg(salary) from employees);
 
 List All Employees whose salary is less than the salary of all the employees working for department_Id 30 */ ------
 ...
 SELECT *,SUM(SALARY) OVER(PARTITION BY DEPARTMENT_ID) AS AVG_SALARY FROM employees;
 SELECT *,SUM(SALARY) OVER(PARTITION BY DEPARTMENT_ID ORDER BY EMPLOYEE_ID) AS SUM_SALARY FROM employees;
 
 SELECT FIRST_NAME, SALARY,
 RANK() OVER(ORDER BY SALARY DESC) AS rankd,
 DENSE_RANK() OVER(ORDER BY SALARY DESC) As dense_rank,
 ROW_NUMBER() OVER(ORDER BY SALARY DESC) As row_numbered,
 NTILE(3) OVER(ORDER BY SALARY DESC) AS ntiled 
 FROM employees;
